-- This is a direct copy from LunarVim
...
M.methods.check_backspace = function() return not has_words_before() end

local T = function(str) return vim.api.nvim_replace_termcodes(str, true, true, true) end

local function feedkeys(key, mode) vim.api.nvim_feedkeys(T(key), mode, true) end
    ...
    cmp = {
        ...
        formatting = {
            ...
            format = function(entry, vim_item)
                local max_width = cmp.formatting.max_width
...
function M.setup()
    ...
    if cmp.cmdline.enable then
        for _, option in ipairs(cmp.cmdline.options) do
